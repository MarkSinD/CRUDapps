...
public class ServeController {
    ...
    @GetMapping("/new")
    public String newServe(Model model){
    ...
    @PostMapping("/save")
    public String saveServe(Serve serve, RedirectAttributes redirectAttributes) throws IOException {
    ...
    private String getRedirectURLtoAffectedUser(Serve serve) {
        String firstName = serve.getTitle();
        return "redirect:/serves/page/1?sortField=id&sortDir=asc&keyword=" + firstName;
    }


    @GetMapping("/edit/{id}")
    public String editServe(@PathVariable(name = "id") Integer id,
                           Model model,
    ...
    public String deleteServe(@PathVariable(name = "id") Integer id,
                             Model model,
                             RedirectAttributes redirectAttributes) {
        try {
